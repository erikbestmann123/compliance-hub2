...
export const scopedRegistryPolyfill = () => {
    ...
    class ShimmedCustomElementsRegistry {
        _definitionsByTag = new Map();
        _definitionsByClass = new Map();
        _whenDefinedPromises = new Map();
        _awaitingUpgrade = new Map();
        define(tagName, elementClass) {
            ...
            const formAssociated = standInClass?.formAssociated ??
    ...
    if (document.readyState === 'loading') {
        ...
        document.addEventListener('readystatechange', () => {
            elementsPendingAttributes.forEach((instance) => customizeAttributes(instance, definitionForElement.get(instance)));
        }, { once: true });
    }
    ...
    Element.prototype.attachShadow = function (init, ...args) {
        ...
        const { customElements: customElements, registry: registry = customElements, ...nativeInit } = init;
        const shadowRoot = nativeAttachShadow.call(this, nativeInit, ...args);
    ...
    if (!!window['ElementInternals'] && !!window['ElementInternals'].prototype['setFormValue']) {
        ...
        const RadioNodeList = class extends Array {
            _elements;
            ...
            constructor(elements) {
                ...
                entries.forEach((value, name) => {
                    ...
                    if (value.length === 1) {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        this[name] = new RadioNodeList(value);
                    }
                });
            }
            ['item'](index) {
